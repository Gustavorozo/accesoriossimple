...
class UsuarioController
{
    ...
    public function __construct(array $_FORM)
    {
        ...
        $this->dataUsuario['nombre'] = $_FORM['nombres'] ?? NULL;
        $this->dataUsuario['Correo'] = $_FORM['apellidos'] ?? null;
        $this->dataUsuario['Cedula'] = $_FORM['documento'] ?? NULL;
    ...
    public function create($withFiles = null) {
        try {
            if (!empty($this->dataUsuario['Cedula']) && !Usuario::usuarioRegistrado($this->dataUsuario['cedula'])) {
                if(!empty($withFiles)){
    ...
    static public function selectUsuario(array $params = []) {
        ...
        if (is_array($arrUsuario) && count($arrUsuario) > 0) {
            ...
            foreach ($arrUsuario as $usuario)
                if (!UsuariosController::usuarioIsInArray($usuario->getId(), $params['arrExcluir']))
                    $htmlSelect .= "<option " . (($usuario != "") ? (($params['defaultValue'] == $usuario->getId()) ? "selected" : "") : "") . " value='" . $usuario->getId() . "'>" . $usuario->getCedula() . " - " . $usuario->getNombre() . " " . $usuario->getCorreo() . "</option>";
        }
        $htmlSelect .= "</select>";
        return $htmlSelect;
    }

    private static function usuarioIsInArray($idUsuario, $ArrUsuario)
    {
